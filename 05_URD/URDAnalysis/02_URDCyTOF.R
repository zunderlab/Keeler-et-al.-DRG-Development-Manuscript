...
print("start UrdCyTOF.R")

rm(list = ls())
.libPaths( c( .libPaths(), "/project/zunderlab/R/3.6.3_URD_Debug") )
...
CONCATTRANSFORMED.URD.FILENAME <- "/Concat_Transformed.csv" #.txt file for URD
URD.META.FILENAME <- "/metadata_URD.csv" #.txt file for URD, as generated by GetUrdMetadata.R
PANEL.FILENAME <- "/panel_URD.csv"
ROOT.STAGE.TAG <- NULL #Metadata tag for cells to be considered roots in pseudotime, leave null if 
#clusters defined in metadata, ex input is "E14"
TIP.STAGE.TAG <- NULL #Metadata tag for cells to be considered tips in pseudotime, leave null if 
#clusters defined in metadata, ex input is "P4"
...
#Calculate pseudotime
pseudotime.out <- floodPseudotime(URD_Object,root.cells, n=50, minimum.cells.flooded = 2, verbose=F)
...
## Build tree =====================================================================================
#This section will build a tree then output images of the tree 
#colored by stage, then segments of the tree. Also will output UMAP colored by segment
...
URD_Object.tree <- buildTree(URD_Object.tree, pseudotime = "pseudotime", tips.use=1:num.tips,
                             divergence.method = "preference", cells.per.pseudotime.bin = 25,
                             bins.per.pseudotime.window = 8, save.all.breakpoint.info = T,
                             p.thresh=0.001)

#Name tips
#tip.names <- c("Notochord", "Prechordal Plate") #Uncomment this line to name tips
