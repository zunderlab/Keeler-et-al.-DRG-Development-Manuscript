...
URD_DATA_FILES <- c("URD_1.RData", "URD_2.RData") #URD.RData file, 1 or 2
...
for (u in URD_DATA_FILES) {
  ...
  if (is.null(PARENT_ORDER)) {
    ...
    order_segments(diff_markers, as.numeric(diff_markers[root_index,"parent"]))
  } else {
  ...
  root_segment <- diff_markers[root_index,"parent"] # Get root segment
  ...
  ggsave(dot_png_name, gg_dot_plot, width=PNG_WIDTH, height=PNG_HEIGHT, units="in")
...
ggsave(dot_png_name, p, width=1.3*PNG_WIDTH, height=PNG_HEIGHT, units="in")
