...
LAYOUT_FILENAME <- "UMAP_layout.csv"
CLUST_ASSIGNS_FILENAME <- "clusters.csv"
...
xy <- read.table(LAYOUT_FILENAME, header=TRUE, sep=",",
                 check.names=FALSE)
...
shifts <- read.table(SHIFTS_FILENAME, header=TRUE, row.names=1,
...
if (!is.null(CLUST_ASSIGNS_FILENAME)) {
  clust_color <- read.table(CLUST_ASSIGNS_FILENAME, header=TRUE,
...
plot(x=xy_shifted[,1], y=xy_shifted[,2], pch=".", cex=point_size,
     col=clust_color, axes=FALSE, ylab="", xlab="")
box()
dev.off()